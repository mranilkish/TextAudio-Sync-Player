 ...
 * @(#) ID3v2Frame.java
 *
 * Created on 23.12.2013 by Anil Kishan
 * 
 *-----------------------------------------------------------------------
...
package sm.smcreator.smc.multimedia.mp3.id3;

import sm.smcreator.smc.multimedia.mp3.id3.exceptions.ID3v2FormatException;
 ...
 *
 * @author:  Jonathan Hilliker modified by Anil Kishan
 */
	...
	public String getDataString() throws ID3v2FormatException
	{
		...
		if (frameData.length > 1)
		{
			try
			{
				...
				{
					str = Helpers.retrieveAsString(frameData, 1, frameData.length-1, ENC_TYPES[frameData[0]]);
				}
				else 
				if (id.charAt(0) == 'W')
				{
				...
				if (id.equals(ID3v2Frames.USER_DEFINED_URL))
				{
					...
					{
						str += '\n';
						str += Helpers.retrieveAsString(frameData, str.length()+2, frameData.length-2-str.length(), ENC_TYPES[frameData[0]]);
					}
				}
				else 
				...
				{
					str = Helpers.retrieveAsString(frameData, 4, frameData.length-4, ENC_TYPES[frameData[0]]);
				}
			}
			catch (ArrayIndexOutOfBoundsException e)
			{
				throw new ID3v2FormatException("Frame: " + id + " has not specified a valid encoding type.");
			}
		}
